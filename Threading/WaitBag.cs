...
namespace Micro.Threading {
    public class WaitBag : IDisposable {
        public const ushort MaxLength = ushort.MaxValue - 1;
        public bool ThrowWhenInvalidKey { get; set; } = false;
        public int Length
            => events.Count;
        ConcurrentDictionary<ushort, ManualResetEvent> events;
        Dictionary<ushort, object> results;

        public WaitBag() {
            events = new ConcurrentDictionary<ushort, ManualResetEvent>();
        ...
        ManualResetEvent getTurn(in ushort key) {
